;(function() {
  const x=Function('return this')(); if (!x.fest)x.fest={}; x.fest['components/oneTask.tmpl']=function(__fest_context) {
    ...
    } const data=__fest_context; __fest_buf+=('<div class="container"><img class="avatarTask" src="assets\/logoBadFront.png" alt="Avatar"/><h2 class="titleTaskOnMainPage">'); try {
      __fest_buf+=(__fest_escapeHTML(data.title));
    } catch (e) {
      __fest_log_error(e.message + '6');
    }__fest_buf+=('</h2><h3>Автор:'); try {
      __fest_buf+=(__fest_escapeHTML(data.orderLogin));
    } catch (e) {
      __fest_log_error(e.message + '9');
    }__fest_buf+=('</h3><p>'); try {
      __fest_buf+=(__fest_escapeHTML(data.text));
    } catch (e) {
      __fest_log_error(e.message + '10');
    }__fest_buf+=('</p><h3>Время:'); try {
      __fest_buf+=(__fest_escapeHTML(data.time));
    } catch (e) {
      __fest_log_error(e.message + '11');
    }__fest_buf+=('</h3><h3>Теги:'); try {
      __fest_buf+=(__fest_escapeHTML(data.tag));
    } catch (e) {
      __fest_log_error(e.message + '12');
    }__fest_buf+=('</h3></div>'); __fest_to=__fest_chunks.length; if (__fest_to) {
