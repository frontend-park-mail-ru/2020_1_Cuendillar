;(function() {
  const x=Function('return this')(); if (!x.fest)x.fest={}; x.fest['components/tasks.tmpl']=function(__fest_context) {
    ...
    } const data=__fest_context; let i; let v; let __fest_to0; let __fest_iterator0; try {
      __fest_iterator0=data || []; __fest_to0=__fest_iterator0.length;
    } catch (e) {
      __fest_iterator0=[]; __fest_to0=0; __fest_log_error(e.message);
    } for (i=0; i<__fest_to0; i++) {
      v=__fest_iterator0[i]; __fest_buf+=('<div class="container"><img class="avatarTask" src="assets\/logoBadFront.png" alt="Avatar"/><a id="task1" href="#"><a href="#"><h2 class="titleTaskOnMainPage" id="taskID:'); try {
        __fest_buf+=(__fest_escapeHTML(v.id));
      } catch (e) {
        __fest_log_error(e.message + '13');
      }__fest_buf+=('">'); try {
        __fest_buf+=(__fest_escapeHTML(v.title));
      } catch (e) {
        __fest_log_error(e.message + '17');
      }__fest_buf+=('</h2></a></a><h3>Автор:'); try {
        __fest_buf+=(__fest_escapeHTML(v.orderLogin));
      } catch (e) {
        __fest_log_error(e.message + '19');
      }__fest_buf+=('</h3><p>'); try {
        __fest_buf+=(__fest_escapeHTML(v.text));
      } catch (e) {
        __fest_log_error(e.message + '20');
      }__fest_buf+=('</p><h3>Время:'); try {
        __fest_buf+=(__fest_escapeHTML(v.time));
      } catch (e) {
        __fest_log_error(e.message + '21');
      }__fest_buf+=('</h3><h3>Теги:'); try {
        __fest_buf+=(__fest_escapeHTML(v.tag));
      } catch (e) {
        __fest_log_error(e.message + '22');
      }__fest_buf+=('</h3></div>');
    }__fest_to=__fest_chunks.length; if (__fest_to) {
